...
app.get('/channels/:channelName/chaincodes/:chaincodeName', async function (req, res) {
    try {
        ...
        let args = req.query.data.args;
        let fcn = req.query.data.fcn;
        ...
        logger.debug(args);
        // checkMalware(args);
        const isSafe = await checkMalware(args);
        if(isSafe){
...
// const checkMalware = (url) => {
//     const request = nvt.makeAPI().setKey('883ac9a0536a8742e2a7eed4e3d27a9d0e772e84a31566524054e0b93ffa0b66');
//     request.domainLookup(url, function (err, res) {
//         if (err) {
...
//             if (road && road.data && road.data.attributes && road.data.attributes.last_analysis_results) {
//                 var lastAnalysisResults = road.data.attributes.last_analysis_results;
                
//                 if (lastAnalysisResults.Kaspersky && lastAnalysisResults.Kaspersky.result != "clean") {
